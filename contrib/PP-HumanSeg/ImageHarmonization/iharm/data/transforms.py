...
class HCompose(Compose):
    def __init__(self,
        ...
        super().__init__(
            transforms, *args, additional_targets=additional_targets, **kwargs)
...
class LongestMaxSizeIfLarger(LongestMaxSize):
    ...
    def apply(self, img, interpolation=cv2.INTER_LINEAR, **params):
        if max(img.shape[:2]) < self.max_size:
            return img
        return F.longest_max_size(
            img, max_size=self.max_size, interpolation=interpolation)
