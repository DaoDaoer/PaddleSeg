...
class IHModelWithBackbone(nn.Layer):
    def __init__(
            self,
            model,
            backbone,
        ...
        if mask_fusion == 'rgb':
            ...
            self.mask_conv = nn.Sequential(
                nn.Conv2d(
                    ...
                    stride=2,
                    padding=1,
                    bias=True), ScaleLayer(init_value=0.1, lr_mult=1))
    ...
    def forward(self, image, mask):
        ...
        backbone_mask = paddle.concat((mask, 1.0 - mask), axis=1)
        if self.downsize_backbone_input:
            ...
            backbone_mask = nn.functional.interpolate(
                ...
                mode='bilinear',
                align_corners=True)
        backbone_image = (self.fusion(backbone_image, backbone_mask[:, :1])
                          if self.mask_fusion == 'rgb' else backbone_image)
        backbone_mask_features = self.mask_conv(
            backbone_mask[:, :1]) if self.mask_fusion == 'sum' else None
        backbone_features = self.backbone(backbone_image, backbone_mask,
