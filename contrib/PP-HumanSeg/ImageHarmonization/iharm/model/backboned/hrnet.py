...
class HRNetIHModel(IHModelWithBackbone):
    def __init__(self,
        ...
        backbone = HRNetBB(
            ...
            pyramid_depth=min(
                depth - 2 if not downsize_hrnet_input else depth - 3, 4),
            width=width,
            ocr=ocr,
            small=small,
            lr_mult=lr_mult,
        ...
        super(HRNetIHModel, self).__init__(base_model, backbone,
                                           downsize_hrnet_input, mask_fusion)


class HRNetBB(nn.Layer):
    def __init__(
            self,
            cat_outputs=True,
        ...
        if self.pyramid_on:
            downsize_in_channels = ocr * 2 if self.ocr_on else sum(
                hrnet_cat_channels)
            self.downsize = MaxPoolDownSize(downsize_in_channels,
                                            pyramid_channels, pyramid_channels,
                                            pyramid_depth)
