...
# import paddle._utils

from .ocr import SpatialOCR_Layer, SpatialGather_Layer
from iharm.utils.log import logger

relu_inplace = True


class HighResolutionLayer(nn.Layer):
    def __init__(self,
                 num_branches,
                 blocks,
                 num_blocks,
...
class HighResolutionNet(nn.Layer):
    def __init__(self,
        ...
        if self.ocr_on:
            ...
            self.ocr_gather_head = SpatialGather_Layer(num_classes)

            self.ocr_distri_head = SpatialOCR_Layer(
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            nn.Conv2d(
                                ...
                                stride=2,
                                padding=1,
                                bias=False), self.norm_layer(outchannels),
                            nn.ReLU(inplace=relu_inplace)))
    ...
    def compute_hrnet_feats(self, x, additional_features, return_list=False):
        ...
        return paddle.concat([x[0], x1, x2, x3], 1)
