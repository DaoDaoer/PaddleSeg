...
class SeparableConv2d(nn.Layer):
    def __init__(self,
        ...
        self.body = nn.Sequential(
            ...
            if norm_layer is not None else nn.Identity(), _activation())
...
class GaussianSmoothing(nn.Layer):
    ...
    def __init__(self, channels, kernel_size, sigma, padding=0, dim=2):
        ...
        meshgrids = paddle.meshgrid(
            [paddle.arange(size, dtype=paddle.float32) for size in kernel_size])
        for size, std, grid in zip(kernel_size, sigma, meshgrids):
            mean = (size - 1) / 2.
            kernel *= paddle.exp(-(
                (grid - mean) / std)**2 / 2) / (std * (2 * math.pi)**0.5)
        # Make sure sum of values in gaussian kernel equals 1.
        kernel = kernel / paddle.sum(kernel)
        # Reshape to depthwise convolutional weight.
        kernel = kernel.reshape(1, 1, *kernel.size())
        kernel = paddle.repeat_interleave(kernel, channels, 0)
...
class MaxPoolDownSize(nn.Layer):
