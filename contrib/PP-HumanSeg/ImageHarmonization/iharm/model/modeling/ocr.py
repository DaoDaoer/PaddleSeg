...
class SpatialGather_Layer(nn.Layer):
    """
    ...
    def __init__(self, cls_num=0, scale=1):
        super(SpatialGather_Layer, self).__init__()
        self.cls_num = cls_num
        self.scale = scale

    def forward(self, feats, probs):
        batch_size, c, h, w = probs.size(0), probs.size(1), probs.size(
            2), probs.size(3)
        probs = probs.reshape(batch_size, c, -1)
        feats = feats.reshape(batch_size, feats.size(1), -1)
...
class SpatialOCR_Layer(nn.Layer):
    """
    Implementation of the OCR Layer:
    ...
    def __init__(self,
                 ...
                 align_corners=True):
        super(SpatialOCR_Layer, self).__init__()
...
class ObjectAttentionBlock2D(nn.Layer):
    ...
    def forward(self, x, proxy):
        ...
        context = context.reshape(batch_size, self.key_channels, *x.size()[2:])
