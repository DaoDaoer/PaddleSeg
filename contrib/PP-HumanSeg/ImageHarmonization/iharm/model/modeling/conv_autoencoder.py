...
class ConvEncoder(nn.Layer):
    def __init__(self,
        ...
        backbone_channels = [] if backbone_channels is None else backbone_channels[::
                                                                                   -1]
        ...
        self.blocks_connected = nn.LayerDict()
        self.connectors = nn.LayerDict()
        for block_i in range(2, depth):
            ...
            if 0 <= backbone_from <= block_i and len(backbone_channels):
                ...
                connector = FeaturesConnector(backbone_mode, in_channels,
            ...
            self.blocks_connected[f'block{block_i}'] = ConvBlock(
                ...
                if 0 <= batchnorm_from <= block_i else None,
                padding=int(block_i < depth - 1))
            self.blocks_channels += [out_channels]

    def forward(self, x, backbone_features):
        backbone_features = [] if backbone_features is None else backbone_features[::
                                                                                   -1]
...
class DeconvDecoder(nn.Layer):
    def __init__(self,
        ...
        self.deconv_blocks = nn.LayerList()

        in_channels = encoder_blocks_channels.pop()
        out_channels = in_channels
        for d in range(depth):
            out_channels = encoder_blocks_channels.pop() if len(
                encoder_blocks_channels) else in_channels // 2
    ...
    def forward(self, encoder_outputs, image, mask=None):
        ...
        for block, skip_output in zip(self.deconv_blocks[:-1],
                                      encoder_outputs[1:]):
        ...
        if self.image_fusion:
            attention_map = paddle.sigmoid(3.0 * self.conv_attention(output))
            output = attention_map * image + (
                1.0 - attention_map) * self.to_rgb(output)
        else:
