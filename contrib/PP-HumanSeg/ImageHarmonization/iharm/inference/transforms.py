...
class PadToDivisor(EvalTransform):
    ...
    def transform(self, image, mask):
        ...
        image = cv2.copyMakeBorder(
            image, *self._pads, self.border_mode, value=self.fill_value)
        mask = cv2.copyMakeBorder(
            mask, *self._pads, self.border_mode, value=self.fill_value)
    ...
    def _remove_padding(self, tensor):
        ...
        cropped = tensor[self._pads.top:tensor_h - self._pads.bottom, self.
                         _pads.left:tensor_w - self._pads.right, :]
        return cropped
...
class NormalizeTensor(EvalTransform):
    def __init__(self, mean, std, device):
        super().__init__()
        self.mean = paddle.to_tensor(mean).reshape(1, 3, 1, 1).to(device)
        self.std = paddle.to_tensor(std).reshape(1, 3, 1, 1).to(device)
...
class ToTensor(EvalTransform):
    ...
    def transform(self, image, mask):
        image = paddle.to_tensor(
            image, device=self.device, dtype=paddle.float32)
        mask = paddle.to_tensor(mask, device=self.device)
        image.unsqueeze_(0)
...
class AddFlippedTensor(EvalTransform):
    def transform(self, image, mask):
        flipped_image = paddle.flip(image, axis=(3, ))
        flipped_mask = paddle.flip(mask, axis=(3, ))
        image = paddle.concat((image, flipped_image), axis=0)
        mask = paddle.concat((mask, flipped_mask), axis=0)
        return image, mask

    def inv_transform(self, image):
        return 0.5 * (image[:1] + paddle.flip(image[1:], axis=(3, )))
