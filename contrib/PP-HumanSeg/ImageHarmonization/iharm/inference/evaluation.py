...
def evaluate_dataset(dataset, predictor, metrics_hub):
    for sample_i in trange(len(dataset), desc=f'Testing on {metrics_hub.name}'):
        ...
        pred = predictor.predict(
            sample['image'], sample_mask, return_numpy=False)
        torch.cuda.synchronize()
        metrics_hub.update_time(time() - predict_start)

        target_image = torch.as_tensor(
            sample['target_image'], dtype=torch.float32).to(predictor.device)
        sample_mask = torch.as_tensor(
            sample_mask, dtype=torch.float32).to(predictor.device)
        with torch.no_grad():
