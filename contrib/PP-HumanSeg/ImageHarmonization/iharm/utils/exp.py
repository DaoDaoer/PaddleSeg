...
def init_experiment(args):
    ...
    dst_script_path = exp_path / (model_path.stem + datetime.strftime(
        datetime.today(), '_%Y-%m-%d-%H-%M-%S.py'))
    ...
    if cfg.multi_gpu:
        ...
        ngpus = paddle.cuda.device_count()
        assert ngpus == cfg.ngpus
    cfg.device = paddle.set_device(f'gpu:{cfg.gpu_ids[0]}')
